...
public class MemberDAOH2 extends JDBConnect implements MemberDAOH2Interface {
	...
	public MemberVO getMember(Integer id) {
		MemberVO member = new MemberVO();
		try {
			query = String.format("select * from Mission4 where id='%s'", Integer.toString(id));
			stmt = con.createStatement();
			rs = stmt.executeQuery(query);
			while(rs.next()) {
				member.setId(rs.getInt("id"));
	...
	public MemberVO addMember(String name, String pass) {
		...
		try {
			...
			query = String.format("insert into Mission4 (name, pass)"
	...
	public MemberVO updateMember(Integer id, String name, String pass) {
		...
		try {
			query = String.format("update Mission4 set name='%s', pass='%s' where id='%s'",
					name, pass, id);
	...
	public MemberVO deleteMember(Integer id) {
		MemberVO member = getMember(id);
		try {
			query = String.format("delete from Mission4 where id='%s'", Integer.toString(id));
			if(member.getId() == null) return null;
			stmt = con.createStatement();
			stmt.executeUpdate(query);
			stmt.close();
	...
	public Integer getMaxId() {
		MemberVO member = new MemberVO();
		try {
			query = "select id from Mission4 order by 1 DESC limit 1";
			stmt = con.createStatement();
			rs = stmt.executeQuery(query);
			while(rs.next()) {
				member.setId(rs.getInt("id"));
